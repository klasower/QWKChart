...
//    [self.view addGestureRecognizer:tap];
    ...
    [self.tableView registerClass:[QWKOptionsCell class] forCellReuseIdentifier:@"QWKOptionsCell"];
...
- (void)loadMessage
{
    ...
    if (![self.conv getReceiver]) {
        return;
    }
    
    // 组装请求参数
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"teamId"] = @(QWK_IM_TEAM_ID);
    params[@"conversationId"] = [self.conv getReceiver];
    params[@"firstPage"] = @(_firstLoad);
    if (self.uiMsgs && self.uiMsgs.count > 0) {
        //找到第一条文字消息
        for (TUIMessageCellData *cellData in self.uiMsgs) {
            if ([cellData isKindOfClass:TUITextMessageCellData.class] || [cellData isKindOfClass:TUISystemMessageCellData.class] || [cellData isKindOfClass:QWKOptionsCellData.class] || [cellData isKindOfClass:TUIImageMessageCellData.class]) {
                if (cellData.mId != 0) {
                    params[@"msgId"] = @(cellData.mId);
                    break;
                }
            }
        }
...
//    if (_lastMsg) {
//        params[@"msgId"] = _lastMsg[@"id"];
//    }
    
    [JDDataReq start:[NSString stringWithFormat:@"%@/app/im/queryMsgHistory", IM_URL_PREFIX]]
    .params(params)
    .silent(YES)
    .success(^(id  _Nonnull data, id  _Nonnull response, BOOL isCache) {
        NSLog(@"history ~ %@", data);
        NSArray *msgs = data[@"msgList"];
        @strongify(self)
        if(msgs.count != 0){
            self.lastMsg = msgs[msgs.count - 1];
        }
        NSMutableArray *uiMsgs = [self qwk_transUIMsgFromIMMsg:msgs];
        dispatch_async(dispatch_get_main_queue(), ^{
            if(![data boolForKey:@"hasNext"]){
                self.noMoreMsg = YES;
...
- (NSMutableArray *)qwk_transUIMsgFromIMMsg:(NSArray *)msgs {
    NSMutableArray *uiMsgs = [NSMutableArray array];
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        
        // qwk
        
        NSDictionary *message = msgs[k];
        ...
        TUISystemMessageCellData *dateMsg = [self transSystemMsgFromDate:[NSDate dateWithFormatString:@"yyyy-MM-dd HH:mm:ss" dateString:message[@"sendAt"]]];
        NSDate *date = [NSDate dateWithFormatString:@"yyyy-MM-dd HH:mm:ss" dateString:message[@"sendAt"]];
        if(_msgDate == nil || fabs([date timeIntervalSinceDate:_msgDate]) > MAX_MESSAGE_SEP_DLAY){
        ...
        if (type == 4) {
            ...
            // 图片
//            TUIImageMessageCellData *data = [[TUIImageMessageCellData alloc] initWithDirection:(isSelf ? MsgDirectionOutgoing : MsgDirectionIncoming)];
//            data.url = message[@"content"];
//            data.mId = msgId;
//            data.avatarUrl = [NSURL URLWithString:isSelf ? [QWKAccount instance].hasLogined?[QWKAccount instance].faceUrl:@"" : [[message objectForKey:@"fromUser"] stringForKey:@"faceUrl"]];
            [uiMsgs addObject:imageData];
        }else {
            if (fromType == 1) {
...
- (NSMutableArray *)transUIMsgFromIMMsg:(NSArray *)msgs
{
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        for (int i = 0; i < msg.elemCount; ++i) {
            ...
            }else if(![elem isKindOfClass:[TIMCustomElem class]] &&![[[msg getConversation] getReceiver] isEqualToString:[_conv getReceiver]]){
                continue;
            }
            hasShowElem =true;
        }
...
- (void)sendMessage:(TUIMessageCellData *)msg
{
    ...
    msg.avatarUrl = [NSURL URLWithString:[QWKAccount instance].hasLogined?[QWKAccount instance].faceUrl:@""];
    ...
    if ([msg isKindOfClass:TUIImageMessageCellData.class]) {
        TUIImageMessageCellData *imgCellData = (TUIImageMessageCellData *)msg;
        [TUIMessageController uploadImages:@[[UIImage imageWithContentsOfFile:imgCellData.path]] success:^(NSURLSessionDataTask *task, id responseObject, BOOL cache) {
            NSString *url = [responseObject stringForKey:@"url"];
            if (url) {
                [JDDataReq start:[NSString stringWithFormat:@"%@/app/im/sendMsg", IM_URL_PREFIX]]
                .param(@"teamId", @(QWK_IM_TEAM_ID))
                .param(@"conversationId", self.conv.getReceiver)
                .param(@"content", [NSString stringWithFormat:@"{\"imgUrl\":\"%@\"}", url])
                .param(@"type", @4)
                .success(^(id  _Nonnull data, id  _Nonnull response, BOOL isCache) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        msg.mId = [data integerForKey:@"id"];
                        [ws changeMsg:msg status:Msg_Status_Succ];
                    });
                })
                .error(^(NSString * _Nonnull desc, DataReqErrorType errType, id  _Nonnull data) {
                    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        .param(@"teamId", @(QWK_IM_TEAM_ID))
        .param(@"conversationId", self.conv.getReceiver)
        .param(@"content", ((TUITextMessageCellData *)msg).content)
        .param(@"type", @1)
        .param(@"recordMsg", ((TUITextMessageCellData *)msg).ignoreRecordMsg ? @"false":@"true")
        .success(^(id  _Nonnull data, id  _Nonnull response, BOOL isCache) {
            dispatch_async(dispatch_get_main_queue(), ^{
                msg.mId = [data integerForKey:@"id"];
                [ws changeMsg:msg status:Msg_Status_Succ];
...
+ (void)uploadImages:(NSArray *)images success:(requestSuccessBlock)bloc failure:(requestFailureBlock)failure {
    ...
    for (UIImage *image in images) {
        ...
        dispatch_async(dispatch_get_global_queue (DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSData *imageData = [image bs_dataSmallerThan:1024*1024];
            NSUInteger size = imageData.length;
            NSString *fileName = [NSString stringWithFormat:@"%ld.jpg", time];
            
            NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
            [parameters setObject:fileName forKey:@"name"];
            [parameters setObject:@(QWK_IM_TEAM_ID) forKey:@"teamId"];
            
            [JDDataHelper postWithUrl:[NSString stringWithFormat:@"%@/app/im/uploadImgToken", IM_URL_PREFIX] parameters:parameters success:^(NSURLSessionDataTask *task, id responseObject, BOOL cache) {
                
                int code = [responseObject intForKey:@"code"];
                NSDictionary *data = [responseObject objectForKey:@"data"];
                NSString *token = [data stringForKey:@"token"];
                if (0 == code && token.length > 0) {
...
//            if (imageData.length>10*1024*1024) {//10M以及以上
//                imageData= [UIImage compressImageSize:image toByte:1024*1024*1];
